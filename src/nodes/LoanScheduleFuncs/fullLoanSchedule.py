...
@knext.node(name="Full Loan Schedule",
            node_type=knext.NodeType.MANIPULATOR,
            icon_path="src\icons\LOANSCHEDULE.png",
...
class AmortizationFullScheduleNode:
    """Generates a comprehensive amortization schedule with all payment components and tracking metrics.

# Full Loan Schedule Node

This node creates a complete amortization schedule that includes all essential loan payment components for every period. The schedule provides detailed breakdowns of payments, cumulative totals, remaining balances, and outstanding amounts, making it the most comprehensive loan analysis tool available.
...
**Cumulative_Payments**: Running total of all payments made from period 1 through current period
...
**Outstanding_Payments**: Total payments remaining to be made after current period

**Outstanding_Interest**: Total interest remaining to be paid after current period

**Outstanding_Principal**: Remaining principal balance (same as Remaining_Balance)
"""
    ...
    def configure(self, configure_context, input_schema):
        return input_schema.append([
            ...
            knext.Column(knext.double(), "Cumulative_Payments"),
    ...
    def execute(self, exec_context, input_table):
        ...
        output_columns = input_columns + [
            'Period', 'PMT', 'IPMT', 'PPMT', 'Cumulative_Payments',
            'Cumulative_Interest_Paid', 'Cumulative_Principal_Paid', 'Outstanding_Payments',
        ...
        for _, row in df.iterrows():
            ...
            # Calculate total amounts for outstanding tracking
            total_payments = abs(pmt) * nper
            total_interest = total_payments - pv
        ...
        result_df['Period'] = result_df['Period'].astype(np.int32)
        numeric_columns = [
            'PMT', 'IPMT', 'PPMT', 'Cumulative_Payments',
            'Cumulative_Interest_Paid', 'Cumulative_Principal_Paid','Outstanding_Payments',
            'Outstanding_Interest', 'Outstanding_Principal'
        ]
