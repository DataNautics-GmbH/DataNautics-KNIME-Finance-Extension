...
@knext.node(name="CUMPRINC",
            node_type=knext.NodeType.MANIPULATOR,
            icon_path="src\icons\CUMPRINC.png",
...
@knext.output_table(name="CUMPRINC Results", description="Table with calculated cumulative principal payments")
class CUMPRINCNode:
    """Calculates the cumulative principal payments made between two specified periods of a loan.
    
    # CUMPRINC Node
    
    This node implements Excel's **CUMPRINC** function (equivalent to CUMPPMT) to calculate the total principal reduction achieved over a range of payment periods. This function helps track loan balance reduction and is valuable for equity analysis and loan progress monitoring.
    ...
    **Start Period Column**: Select the column containing the first period number to include in the cumulative principal calculation. Must be between 1 and the total number of periods.
    
    **End Period Column**: Select the column containing the last period number to include in the cumulative principal calculation. Must be greater than or equal to the start period and not exceed the total number of periods.
    ...
    **Payment Timing**: Configure when payments are due within each period. Set to `0` for end-of-period payments (default) or `1` for beginning-of-period payments.
    ...
    The node appends a new **CUMPRINC** column containing the sum of all principal payments from the start period through the end period (inclusive). Values are typically negative, representing the total principal reduction over the specified range.
    ...
    def configure(self, configure_context, input_schema):
        # Check if CUMPRINC column already exists and remove it if it does
        output_schema = input_schema
        if "CUMPRINC" in input_schema.column_names:
            output_schema = input_schema.remove("CUMPRINC")
        return output_schema.append(knext.Column(knext.double(), "CUMPRINC"))

    def execute(self, exec_context, input_table):
        df = input_table.to_pandas()
        
        # Remove CUMPRINC column if it already exists
        if "CUMPRINC" in df.columns:
            df = df.drop(columns=["CUMPRINC"])
        
        def calculate_cumprinc(row):
            return np.sum([
                npf.ppmt(row[self.rate_column], per, row[self.nper_column], 
                        row[self.pv_column], when=self.pmt_type)
                for per in range(int(row[self.start_period_column]), 
                               int(row[self.end_period_column]) + 1)
            ])
        
        df["CUMPRINC"] = df.apply(calculate_cumprinc, axis=1)
